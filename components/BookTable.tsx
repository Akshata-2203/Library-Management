...
const BookTable: React.FC<Props> = ({ books, onAction, actionLabel, showAvailability, loanCounts }) => (
  <div className="overflow-hidden bg-white rounded-4xl border border-slate-100 shadow-sm">
      ...
      <tbody className="divide-y divide-slate-50">
        {books.map((book) => (
          <tr key={book["New NO."]} className="group hover:bg-violet-50/30 transition-colors">
            ...
            {loanCounts && (
              <td className="px-8 py-6">
                <span className={`px-4 py-1.5 inline-flex items-center justify-center gap-2 min-w-27.5 text-[10px] leading-5 font-black uppercase tracking-widest rounded-full border ${loanCounts[book["New NO."]] ? 'bg-amber-50 text-amber-600 border-amber-100' : 'bg-slate-50 text-slate-400 border-slate-100'}`}>
            ...
            {showAvailability && (
              <td className="px-8 py-6">
                <span className={`px-4 py-1.5 inline-flex items-center justify-center min-w-22.5 text-[10px] leading-5 font-black uppercase tracking-widest rounded-full ${book.isAvailable ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'}`}>
